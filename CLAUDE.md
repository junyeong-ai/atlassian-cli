...
- **Error contract**: a failed run prints a single-line JSON object to stderr — `{"error":{"message",...}}`, plus `status`/`operation`/`hint` fields when the failure is a typed `ApiError` — and exits with a stable code: 1 generic, 2 CLI usage (clap), 3 auth (401/403), 4 not found (404), 5 rate limited (429), 6 server error (5xx). Stdout carries results only. `auth status` is the one command that prints on stdout *and* exits non-zero: it reports the configured method and whatever of the stored session it could establish, then fails because part of the answer is missing — a status nobody could read is not a status of "none".
