...
- **Error contract**: a failed run prints a single-line JSON object to stderr — `{"error":{"message",...}}`, plus `status`/`operation`/`hint` fields when the failure is a typed `ApiError` — and exits with a stable code: 1 generic, 2 CLI usage (clap), 3 auth (401/403), 4 not found (404), 5 rate limited (429), 6 server error (5xx). Stdout carries results only. A command that has already printed can still fail: `auth status` reports the configured method and whatever of the stored session it could establish before failing on the part it could not, `config list` prints the header before a path it cannot read, and a `--stream` search has already emitted JSONL when a later page fails. Read the exit code, not the presence of output.
...
- **Destructive-op guard**: whole-resource deletes (`jira delete`, `confluence delete`, `self uninstall`, `self skill remove`) require an explicit `--yes` at the CLI layer (the binary is non-interactive/JSON-first, so a prompt would hang pipelines — a required flag is the guard). The API functions (`delete_issue`/`delete_page`) stay pure; the `--yes` check lives in the `main.rs` handler. Targeted sub-resource removals do **not** require `--yes`, because naming the one thing to remove is itself the specificity guard: Jira `comment delete` and `worklog remove` and Confluence `comment delete`, `label remove`, `property delete` each take the id or name; `watcher remove` reaches only the caller's own watch; and `link remove` either names a link's id or withholds the removal until the issue pair and type resolve to exactly one link. Jira issue delete is irreversible (no recycle bin); Confluence page delete goes to trash.

`jira link remove` therefore takes two forms, mutually exclusive at the parse: the issue pair (with `--type` where more than one link type runs between them), or `--id`. The pair is a description that has to resolve, and `remove_link` refuses rather than guesses wherever it resolves to more than one link or to entries it could not read — states the API reaches, since it will create a second link identical to an existing one. `--id` is the way through every one of those refusals, so it takes no walk at all: the id is the endpoint's whole address, and confirming it against the pair would put the escape back behind the read it exists to bypass. Each refusal names it, and names the ids to choose between wherever the walk read all of them.
...
4. **URL safety**: percent-encode user input in path segments via `http_utils::encode_path_segment`. Use the reqwest `.query(&[(k, v)])` builder for query params containing user input — never `format!` user input into the URL string. Encode a server-returned identifier too where it reaches a path segment — a Confluence comment id does, through `CommentFamily`. The one exception is the cloud ID, which is not encoded because `config::validate_cloud_id` has already narrowed it to `[A-Za-z0-9-]`.
5. **Pagination**: `startAt`/`maxResults` Jira endpoints must use the shared `paginate` helper, naming the `PageContract` they follow — `AGILE_PAGE` (`values` + `isLast`), `COMMENT_PAGE` (`comments` + `total`) or `WORKLOG_PAGE` (`worklogs` + `total`). All bail on a page missing its items or its end signal rather than silently truncating; a new contract is a new constant, never a second loop. Confluence v2 list endpoints are cursor-paginated — route them through `fetch_all_v2_results` instead (see `src/confluence/CLAUDE.md`). Either way, never return only the first page.
...
9. Read endpoints apply the response filter to what they return, never to the transport body they read from. A list filters each item and leaves the `{"items": [...]}` wrapper alone — through `jira::list_envelope` or `confluence::v2_list_envelope` where one fits, inline where the read builds its own envelope (both `search_all`s, Confluence `search`), and not at all for comment listings, which `ThreadWalk::admit` filtered before stamping (see `src/confluence/CLAUDE.md`). A single-object read calls `filter::apply` on the object. Anything a walk or an internal decision reads — an end signal, an `id` a resolver picks by — comes off the raw response first.
