...
- `save` commits to one backend and then clears the other so a later read
  ...
  `load` reads the keychain first, so a file entry left behind by a keychain
  save is shadowed and its warning says only that the clear failed — what that
  clear failed on is reading the file, so what the file holds is the one thing
  unknown there. A keychain entry left behind by a file save is what a later
  read finds, and after rotation its refresh token is the revoked one; that
  warning names the consequence and the remedy, because neither is resolvable
  from here — the keychain that would not answer is the one holding it.
