...
`ThreadWalk` keeps a `seen` set of ids. A tree cannot reach a comment twice, so an id returning as its own descendant is drift and bails — the same posture `CursorTrail` takes toward a cursor that stops advancing. There is no depth cap; one would turn a legitimately deep thread into a failure. What `seen` cannot terminate is a server answering every `children` request with ids it has not used before, so `MAX_THREAD_COMMENTS` bounds the walk as a whole, set far beyond any real page and failing loudly like `paginate`'s `MAX_PAGES` rather than truncating.
