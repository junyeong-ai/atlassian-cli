...
- **`search` items are heterogeneous.** A v1 result carrying a `content` object contributes that object; one that does not — v1 answers space and user entities too — contributes the result as it came. Dropping the latter reported no matches for a query that had them, and in `--all` the count left behind then ended the crawl. So `.items[].id` resolves for content hits and not for the rest, and `count` is the number of matches, not the number of pages of content.
